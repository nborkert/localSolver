		...
		if player.Position == "QB" {
			for _, WR := range allPlayers {
				if WR.Position == "WR" && player.Team == WR.Team {
					stack := solver.PlayerStack{player.PlayerName + " + " + WR.PlayerName, player.Team, player.ProjectedPoints + WR.ProjectedPoints, (player.ProjectedPoints + WR.ProjectedPoints) / float64(player.Salary+WR.Salary)}
					//fmt.Println(stack)
